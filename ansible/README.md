...
Image publication is a separate play. `playbooks/build-image.yml` reads the repository's most recent Git tag and checks for that tag in `notes_image_repository`. If it is missing, the controller builds the root `Dockerfile` locally with Buildx and pushes the tagged image. If it already exists, no build or push occurs.
...
Copy `inventories/example` and edit all environment-specific values. Set `notes_image_repository` to a public writable registry repository. Authenticate Docker on the controller beforehand or provide registry credentials via Ansible Vault. Store `workspace_notes_auth_secret`, registry credentials, and OAuth credentials with Ansible Vault; do not commit real secrets. For Google OAuth, authorize:
